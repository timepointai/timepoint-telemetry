...
fn load_vectors() -> Vec<Vector> {
    let mut paths: Vec<PathBuf> = fs::read_dir(vectors_dir())
        ...
        // Envelope vectors only. The verdict corpora have their own shapes and
        // their own walkers: classification-verdicts.json (§4) is walked by
        // python/test_classification_vectors.py, relation-verdicts.json by
        // tests/relations.rs and python/test_relation_vectors.py.
        .filter(|p| {
            p.file_name().is_some_and(|n| {
                n != "classification-verdicts.json" && n != "relation-verdicts.json"
            })
        })
        .collect();
    paths.sort();
